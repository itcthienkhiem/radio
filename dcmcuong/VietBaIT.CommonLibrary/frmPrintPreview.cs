...
namespace VietBaIT.CommonLibrary
{
	...
	private void cmdExcel_Click(object sender, System.EventArgs e)
	{
		...
		try {
			...
			if (SaveFileDlg.ShowDialog() == DialogResult.OK) {
				...
				if (Utility.AcceptQuestion("Đã xuất dữ liệu thành công ra file Excel ở đường dẫn: " + sFileName + Constants.vbCrLf + "Bạn có muốn mở file Excel ra xem không?","Thông báo")) {
    ...
    private void cmdTieuDe_Click(object sender, EventArgs e)
    {
        frm_TIEU_DE frm=new frm_TIEU_DE();
        frm.txtBaoCao.Text = RptDoc.ToString();
        frm.txtTieuDe.Text = sTIEUDE;
        frm.ShowDialog();
    }
