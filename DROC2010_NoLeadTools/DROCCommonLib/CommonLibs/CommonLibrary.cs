...
namespace VietBaIT.CommonLibrary
{
    ...
    /// </summary>
    ///  public enum TabMode { WorkList = 0, Acq = 1, StudyList = 2, System = 3, Viewer = 4, Print = 5, Option = 6 };
    ...
    public class Utility
    {
        ...
        public static bool IsCheckConnected(string strservers, string DbName, string users, string password)
        {
            ...
            strOleDb += "packet size=4096;workstation id=" + strservers;
            strOleDb += ";data source=" + strservers;
            strOleDb += " ;persist security info=False;initial catalog=" + DbName;
            strOleDb += ";uid=" + users;
            strOleDb += ";pwd=" + password;
            OleDbConnection cnn = new OleDbConnection(strOleDb);
            ///Mở CSDL
            try
            {
        ...
        //Chỉnh lại tên tham số trên OleDb phải có chữ @
        public static string CorrectParameterNameOfOleDbServer(string ParameterName)
        {
        ...
        public static string sGetConditionFromDelegate(string sDelegateUser)
        {
            ...
            {
                Reval += " UCASE(CreatedBy)='" + arrDelegateUser[i].ToString().ToUpper() + "' OR ";
            }
            if (Reval != "")
            {
        ...
        public static string sGetName(string pv_sTableName, string FieldToTake, string pv_sFieldName, int pv_ID)
        {
            try
            {
                OleDbDataAdapter fv_da = new OleDbDataAdapter("SELECT " + FieldToTake + " FROM " + pv_sTableName + " WHERE " + pv_sFieldName + "=" + pv_ID, globalVariables.OleDbConnection);
        ...
        public static System.DateTime getSysDate()
        {
            try
            {
                OleDbDataAdapter DA = new OleDbDataAdapter("SELECT GETDATE()", globalVariables.OleDbConnection);
        ...
        public static string sGetFieldValue(string pv_sTableName, string pv_sFieldName, string pv_sCondition)
        {
            try
            {
                OleDbDataAdapter fv_da = new OleDbDataAdapter("SELECT " + pv_sFieldName + " FROM " + pv_sTableName + " WHERE " + pv_sCondition, globalVariables.OleDbConnection);
                DataTable fv_dt = new DataTable();
                fv_da.Fill(fv_dt);
                if (fv_dt.Rows.Count > 0)
                {
        ...
        {
            OleDbDataAdapter sv_Da = new OleDbDataAdapter("SELECT * FROM " + pv_sTableName, globalVariables.OleDbConnection);
        ...
        {
            OleDbDataAdapter sv_Da = new OleDbDataAdapter("SELECT * FROM " + pv_sTableName + " ORDER BY intSTT ASC", globalVariables.OleDbConnection);
            DataTable sv_Dt = new DataTable();
            try
            {
                sv_Da.Fill(sv_Dt);
        ...
        {
            DataTable fv_dt = new DataTable();
            OleDbDataAdapter fv_da = new OleDbDataAdapter("SELECT MAX(" + pv_sFieldName + ") FROM " + pv_sTableName, globalVariables.OleDbConnection);
            try
            {
        ...
        {
            DataTable fv_dt = new DataTable();
            OleDbDataAdapter fv_da = new OleDbDataAdapter("SELECT MAX(" + pv_sFieldName + ") FROM " + pv_sTableName + " where Parent_ID=" + Val, globalVariables.OleDbConnection);
            try
            {
                fv_da.Fill(fv_dt);
                if (fv_dt.Rows.Count > 0)
        ...
        /// <param name="pv_sTableName"></param>
        /// <returns></returns>
        public static int GetValueOfField(string pv_sFieldName, string pv_sTableName, string Condition)
        {
            DataTable fv_dt = new DataTable();
            OleDbDataAdapter fv_da = new OleDbDataAdapter("SELECT " + pv_sFieldName + " FROM " + pv_sTableName + " where " + Condition, globalVariables.OleDbConnection);
            try
            {
                fv_da.Fill(fv_dt);
                if (fv_dt.Rows.Count > 0)
                {
                    return (Utility.Int32Dbnull(fv_dt.Rows[0][0]));
                }
                else
                {
                    return -11;
        ...
        public static ActionResult DeleteValueFromTable(string TableName, string Condition)
        {
            try
            {
        ...
        {
            OleDbDataAdapter fv_DA = null;
            if (pv_MoneyID != -1)
            {
                fv_DA = new OleDbDataAdapter("SELECT fRate FROM TBL_MONEYUNIT WHERE ID=" + pv_MoneyID, globalVariables.OleDbConnection);
            }
            else
            {
                fv_DA = new OleDbDataAdapter("SELECT fRate FROM TBL_MONEYUNIT WHERE sName='" + pv_sMoneyName + "'", globalVariables.OleDbConnection);
            }
        ...
        public static bool bIsExisted(string pv_sTableName, string pv_sFieldName, object pv_objValue, bool pv_bIsDigit)
        {
            try
            {
                OleDbDataAdapter fv_DA = new OleDbDataAdapter("SELECT * FROM " + pv_sTableName + " WHERE " + pv_sFieldName + sGetCondition(pv_objValue, pv_bIsDigit), globalVariables.OleDbConnection);
                DataSet fv_DS = new DataSet();
        ...
        public static bool bIsExisted(string pv_sTableName, string sCondition)
        {
            try
            {
                OleDbDataAdapter fv_DA = new OleDbDataAdapter("SELECT 1 FROM " + pv_sTableName + " WHERE " + sCondition, globalVariables.OleDbConnection);
                DataSet fv_DS = new DataSet();
                fv_DA.Fill(fv_DS, pv_sTableName);
                if (fv_DS.Tables[0].Rows.Count > 0)
                {
        ...
        public static string sGetCondition(object pv_objValue, bool pv_bIsDigit)
        {
            ...
            {
                return " ='" + Utility.sDbnull(pv_objValue) + "'";
            }
        }
        /// <summary>
        ...
        {
            OleDbDataAdapter da = new OleDbDataAdapter("select max(Left(RegNo+'0',1)) from TBL_REGSERVICE where RIGHT('000'+REGNO,3)<='999' AND RIGHT(LEFT(REGNO+'0000000',7),6)='" + yymmdd + "'", globalVariables.OleDbConnection);
            DataTable dt = new DataTable();
            try
            {
                da.Fill(dt);
        ...
        {
            OleDbDataAdapter da = new OleDbDataAdapter("select 1 from TBL_REGSERVICE where RIGHT('000'+REGNO,3)='999' AND LEFT(REGNO+'0000000',7)='" + CYYMMDD + "'", globalVariables.OleDbConnection);
            DataTable dt = new DataTable();
            try
            {
                da.Fill(dt);
        ...
        {
            string fv_sOleDb = "";
            fv_sOleDb = "SELECT " + pv_sFieldName + " FROM " + pv_sTableName + " WHERE " + pv_sCondition;
            OleDbDataAdapter DA = new OleDbDataAdapter(fv_sOleDb, globalVariables.OleDbConnection);
            DataTable DT = new DataTable();
            try
            {
                DA.Fill(DT);
                    ...
                    for (int i = 0; i <= fi.Length - 1; i++)
                    {
                        ...
                        switch (f.Name)
                        {
                            ...
                            case "Branch_Website":
                                ...
                                break;
                            case "globalVariables.OleDbConnection":
                                SetField(t, "globalVariables.OleDbConnection", globalVariables.OleDbConnection);
                                break;
                            case "Branch_ID":
 ...
 /// <summary>
    ...
    public class TraceController
    {
        ...
        public ActionResult Save(System.Data.SqlClient.SqlTransaction Trans)
        {
            ...
            string OleDbStatement = "INSERT INTO Sys_TRACE(BranchID,UserName,CreatedDate,IPAddress,ComputerName,AccountName,DLLName,SubSystemName,FunctionID,FunctionName,TableName,[Desc],LOT)";
            OleDbStatement += " VALUES('" + Infor.BranchID + "','" + Infor.UserName + "',CDate('" + Infor.CreatedDate + "'),'";
            OleDbStatement += Infor.IPAddress + "','" + Infor.ComputerName + "','" + Infor.AccountName + "','" + Infor.DLLName + "','" + Infor.SubSystemName + "'," + Infor.FunctionID + ",'" + Infor.FunctionName + "','" + Infor.TableName + "','" + Infor.Desc + "'," + Infor.LOT + ")";

            try
            {
                return ActionResult.Success;// DataAccess.ExecuteNonQuery(Trans, CommandType.Text, OleDbStatement) > 0 ? ActionResult.Success : ActionResult.Error;
    ...
    public class cls_SignInfor
    {
        ...
        {
            exists = true;
            OleDbCommand cmdNguoiKy = new OleDbCommand();
            DataTable dtbNguoiKy = new DataTable();
            try
            {
                {
                    cmdNguoiKy.Connection = globalVariables.OleDbConnection;
                    cmdNguoiKy.CommandType = CommandType.Text;
                    cmdNguoiKy.CommandText = "Select * from Sys_trinhKy Where ReportName = '" + sv_sBieuMau + "'";
                }
                OleDbDataAdapter adtNguoiKy = new OleDbDataAdapter(cmdNguoiKy);
        ...
        //    if (exists == true)
        //    {
        //        strOleDb = "Update Sys_TRINHKY set";
        //        strOleDb += " OBJECTName = '" + mv_TEN_DOITUONG + "'";
        //        strOleDb += " ,Font_Name = '" + mv_FONT_CHU + "'";
        //        strOleDb += " ,Font_Size = " + mv_CO_CHU;
        //        strOleDb += " ,Font_Stype = '" + mv_KIEU_CHU + "'";
        //        strOleDb += " ,ObjectContent = '" + mv_NOI_DUNG + "'";
        //        strOleDb += " Where ReportNAME = '" + mv_TEN_BIEUBC + "'";
        //    }
        //    else
        //    {
        //        strOleDb = "Insert Into Sys_TRINHKY(ReportNAME, ObjectName,  Font_Name, Font_Size, Font_Stype, ObjectContent)";
        //        strOleDb += " Values(";
        //        strOleDb += " '" + mv_TEN_BIEUBC + "'";
        //        strOleDb += " ,'" + mv_TEN_DOITUONG + "'";
        //        strOleDb += " ,'" + mv_FONT_CHU + "'";
        //        strOleDb += " ," + mv_CO_CHU;
        //        strOleDb += " ,'" + mv_KIEU_CHU + "'";
        //        strOleDb += " ,'" + mv_NOI_DUNG + "')";
        ...
        //            cmdNguoiKy.Connection = globalVariables.OleDbConnection;
        //            cmdNguoiKy.CommandType = CommandType.Text;
        //            cmdNguoiKy.CommandText = strOleDb;
...
namespace AppLogger
{
    public class LogAction
    {
        ...
        delegate void SetText(Label lbl, string content);
        ...
        public static void LogActions(string _content)
        {
            try
            {
                ...
                string ActionLog = Application.StartupPath + @"\DROCLogs\ActionLog_" + VietBaIT.CommonLibrary.Utility.GetYYYYMMDD(DateTime.Now) + ".txt";
...
namespace FPDType
{
    public class DRTechFPD_UnifiedEthernet
    {
    ...
    public class ToshibaFPD
    {
        #region Khai báo các biến liên quan đến tấm Toshiba FDX4343R
        public const int TE_OK = 0x0000;
        public const int TE_COMMUERR = 0x0001;
        public const int TE_PERFORMERR = 0x0002;
        public const int TE_MAPERR = 0x0003;
        public const int TE_STATUSERR = 0x0004;
        public const int TE_PARAMERR = 0x0005;
        public const int TE_NOTINIT = 0x0006;
        public const int TE_NOTSUPPORT = 0x0007;


        //correction type
        public const int TE_PROCESS_NONE = 0x0000;
        public const int TE_PROCESS_OFFSET = 0x0001;
        public const int TE_PROCESS_GAIN = 0x0002;
        public const int TE_PROCESS_DEFECTIVE = 0x0004;


        //raw data output type
        public const int TE_IMG_RAW = 0x0001;
        public const int TE_IMG_OFFSET = 0x0002;
        public const int TE_IMG_AFTEROFFSET = 0x0004;
        public const int TE_IMG_AFTERGAIN = 0x0008;
        public const int TE_IMG_CLEAN = 0x0010;

        //detector status
        public const int TE_RadiographyStatus = 0x0001;
        public const int TE_FPDReady = 0x0002;
        public const int TE_FlashMemoryStatus = 0x0004;

        public const int TE_TemperatureSensor1 = 0x0001;
        public const int TE_TemperatureSensor2 = 0x0002;

        public const int TE_TFTONVoltageMonitor = 0x0001;
        public const int TE_TFTOFFVoltageMonitor = 0x0002;
        public const int TE_Correction1VoltageMonitor = 0x0004;
        public const int TE_Correction2VoltageMonitor = 0x0008;
        public const int TE_PDVoltageMonitor = 0x0010;

        //event
        public const int TE_SeqStartEvent = 0x0001;
        public const int TE_XWindowOnEvent = 0x0002;
        public const int TE_XWindowOffEvent = 0x0003;
        public const int TE_ImgTransStartEvent = 0x0004;
        public const int TE_ImgTransEndEvent = 0x0005;
        public const int TE_ImgDataReadyEvent = 0x0006;
        public const int TE_FPDPrePareEvent = 0x0007;
        public const int TE_FPDReadyEvent = 0x0008;
        public const int TE_FPDAbortEvent = 0x0009;
        public const int TE_FPDSleepEvent = 0x000A;
        public const int TE_FPDWakingEvent = 0x000B;
        public const int TE_FPDErrorEvent = 0x000C;
        public const int TE_FPDWarningEvent = 0x000D;
        public const int TE_ImgAcqTimeoutEvent = 0x000E;

        //calibration 
        public const int TE_FPDCalProgressEvent = 0x0020;

        //hardware status notification
        public const int TE_FPDHWStatusEvent = 0x0030;


        //warning code;
        public const int TE_FPDWarningFlatImageCheckFailed = 0x0006;
        public const int TE_FPDWarningDarkImageCheckFailed = 0x0007;

        //error code
        public const int TE_FPDErrorNotConnected = 0x00e6;



        //system state, to tetd
        public const int TE_PANEL_STATE_INITIAL = 0x0001;	//	just after initial;
        public const int TE_PANEL_STATE_STANDBY = 0x0002;	//	stand by mode;
        public const int TE_PANEL_STATE_CALIBRATION = 0x0003;	//	calibration mode;
        public const int TE_PANEL_STATE_WORK = 0x0004;	//	work mode;
        public const int TE_PANEL_STATE_INACTIVE = 0x0005;	//	inactIve mode when the other detector is actived in two detectors system;
        public const int TE_PANEL_STATE_OFFSET = 0x0006;	//	performing offset calibration;


        //exposure mode
        public const int TE_SINGLE_MODE = 0x0000;
        public const int TE_DUAL_ENERGY_MODE = 0x0001;
        public const int TE_OFFSET_MODE = 0x0002;


        public const int TE_MAXCHAR_LENGTH = 256;
        #endregion

        #region Toshiba Structure
        [Serializable]
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
        public struct tagFPDInfo
        {
            public int nColumn;									// image width in pixel
            public int nRow;										// image height in pixel
            public int nBits;										// image depth
            public float fFPDTemperature;							// FPD temperature
            public float fFPDElementPhysicalSize;					// FPD Element Physical Size
            public float fFPDElementSpacing;
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = TE_MAXCHAR_LENGTH)]// FPD Element Spacing
            public string charFPDType;			// FPD Type
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = TE_MAXCHAR_LENGTH)]
            public string charFPDID;				// FPD ID
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = TE_MAXCHAR_LENGTH)]
            public string charFPDBinning;			// FPD Binning
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = TE_MAXCHAR_LENGTH)]
            public string charFPDCalDateTime;		// FPD last calibration date time
        } ;

        [StructLayout(LayoutKind.Sequential)]
        public struct tagImgInfo
        {
            public int nWidth;										// Image Width in pixel
            public int nHeight;									// Image Height in Pixel
            public int nBits;										// Image data depth
            public IntPtr pwData;									// point to image data
        };

        [Serializable]
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
        public struct tagErrInfo
        {
            public int nCode;
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = TE_MAXCHAR_LENGTH)]//	error, warning code
            public string charMeaning;			//	error, warning meaning
        } ;

        [Serializable]
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
        public struct tagCalProgressInfo
        {
            public float fPercentage;								//	the percentage of the calibration process					
        } ;

        [Serializable]
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
        public struct tagFPIHWStatusInfo
        {
            public int nFPDstatus;									//	status
            public int nFPDtemperature;							//	temperature
            public int nFPDvoltage;								//	voltage
        } ;
        #endregion
    }
}
