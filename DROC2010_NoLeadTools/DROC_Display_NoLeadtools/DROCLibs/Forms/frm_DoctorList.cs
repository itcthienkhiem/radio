...
{
    public partial class frm_DoctorList : BaseForm
    {
        #region "Attributes"
        /// <summary>
        ...
        /// </summary>
        private DataTable DoctorEntity = new DoctorEntity.DoctorEntityDataTable();
        //Khai báo gói thông tin chứa DoctorEntity và Header truyền lên BL để xử lý nghiệp vụ
        private DataSet DataEntity = new DataSet();
        /// <summary>
        /// Datasource là danh sách Doctor hiển thị trên lưới
        /// </summary>
        ...
        private void frm_DoctorList_Load(object sender, EventArgs e)
        {
            InitView();
        }
        ...
        private void SetValueforEntity()
        {
            ...
            Utility.ResetEntity(ref DoctorEntity);
            //Create new Row
            DataRow dr = DoctorEntity.NewRow();
            dr["Doctor_Code"] = Utility.GetValue(txtCode.Text, false);
            dr["Doctor_Name"] = Utility.GetValue(txtName.Text, false);
            dr["Desc"] = Utility.GetValue(txtDesc.Text, false);
            dr["Pos"] = Convert.ToInt16(txtPos.Text);
            dr["IsEmerency"] = Convert.ToInt16(chkEmerency.Checked ? 1 : 0);
            //Set giá trị cho cột STT cũ để tráo đổi số TT với bản ghi có STT=dr["Pos"]
            dr["OldPos"] = OldPos;
            DoctorEntity.Rows.Add(dr);
            DoctorEntity.AcceptChanges();
            //Add DoctorEntity into DataEntity
            DataEntity.Tables.Add(DoctorEntity);
        }
        #endregion
        ...
        public void InitView()
        {
            ...
            Initialize();
            //Lấy về danh sách các Bác sĩ để hiển thị lên DataGridView
            GetData();
        ...
        private bool IsValidData()
        {
            ...
            {
               mdlStatic.SetMsg(lblMsg,"Bạn cần nhập mã Bác sĩ.",true);
                txtCode.Focus();
                return false;
            }
            ...
            if (String.IsNullOrEmpty(txtName.Text))
            {
               ...
               mdlStatic.SetMsg(lblMsg,"Bạn cần nhập tên Bác sĩ.",true);
                txtName.Focus();
                return false;
            }
            return true;
        ...
        private void SetControlStatus()
        {
            ...
            {
                case action.Insert:
                    //Cho phép nhập liệu mã Bác sĩ,vị trí, tên Bác sĩ và mô tả thêm
                    txtCode.Clear();
                    txtName.Clear();
                    txtDesc.Clear();
                    ...
                    txtPos.Text = Utility.getNextMaxID("Pos", "L_DoctorList").ToString();
                    ...
                    break;
                case action.Update:
                    //Không cho phép cập nhật lại mã Bác sĩ
                    Utility.DisabledTextBox(txtCode);
                    //Cho phép cập nhật lại vị trí, tên Bác sĩ và mô tả thêm
                    Utility.EnabledTextBox(txtName);
                    Utility.EnabledTextBox(txtDesc);
                    Utility.EnabledTextBox(txtPos);
                    chkEmerency.Enabled = true;
                ...
                case action.FirstOrFinished://Hủy hoặc trạng thái ban đầu khi mới hiển thị Form
                    //Không cho phép nhập liệu mã Bác sĩ, tên Bác sĩ và mô tả thêm
                    Utility.DisabledTextBox(txtCode);
        ...
        public void CurrentCellChanged()
        {
            try
            {
                ...
                if (bLoaded && AllowCurrentCellChangedOnGridView && grdList.RowCount > 0 && grdList.CurrentRow != null)
                {
                    ...
                    {
                        txtCode.Text = Utility.GetValueFromGridColumn(grdList, "colDoctorCode", grdList.CurrentRow.Index);
                        txtName.Text = Utility.GetValueFromGridColumn(grdList, "colDoctorName", grdList.CurrentRow.Index);
                        ...
                        chkEmerency.Checked = (Utility.GetValueFromGridColumn(grdList, "colIsEmerency", grdList.CurrentRow.Index)=="0"?false:true);
                    }
                    else
                    {
                        chkEmerency.Checked = false;
        ...
        /// <summary>
        /// Lấy danh sách Bác sĩ và Binding vào DataGridView
        /// </summary>
        private void GetData()
        {
            DataSource = new DoctorController().GetAllData().Tables[0];
            Utility.SetDataSourceForDataGridView(grdList, DataSource, false, true,"", "Pos,Doctor_Name");
        }
        #endregion
        ...
        private void SearchData(string _Code,string _Name)
        {
            ...
            if(_Code.Trim()!="")
                WhereCondition= " Doctor_Code='" + _Code + "'";
            if (_Name.Trim() != "")
                if (WhereCondition != "") WhereCondition += " AND Doctor_Name like '%" + _Name + "%'";
                else
                    WhereCondition = " Doctor_Name like '%" + _Name + "%'";
            if (_Code.Trim() == "" && _Name.Trim() == "") WhereCondition = " 1=1";
            DataSource = new DoctorController().GetData(WhereCondition).Tables[0];
            Utility.SetDataSourceForDataGridView(grdList, DataSource, false, true, "", "Pos,Doctor_Name");
            CurrentCellChanged();
        }
    }
