...
{
    public partial class frm_ManufactureList : BaseForm
    {
        #region "Attributes"
        /// <summary>
        ...
        /// </summary>
        private DataTable ManufactureEntity = new ManufactureEntity.ManufactureEntityDataTable();
        //Khai báo gói thông tin chứa ManufactureEntity và Header truyền lên BL để xử lý nghiệp vụ
        private DataSet DataEntity = new DataSet();
        /// <summary>
        /// Datasource là danh sách Manufacture hiển thị trên lưới
        /// </summary>
        ...
        private void frm_ManufactureList_Load(object sender, EventArgs e)
        {
            InitView();
        }
        ...
        private void SetValueforEntity()
        {
            ...
            Utility.ResetEntity(ref ManufactureEntity);
            //Create new Row
            DataRow dr = ManufactureEntity.NewRow();
            if (Act == action.Insert) dr["Manufacture_ID"] = -1;
            else dr["Manufacture_ID"] = Convert.ToInt16(txtID.Text.Trim());
            dr["Manufacture_Code"] = Utility.GetValue(txtCode.Text, false);
            dr["Manufacture_Name"] = Utility.GetValue(txtName.Text, false);
            ...
            dr["OldPos"] = OldPos;
            ManufactureEntity.Rows.Add(dr);
            ManufactureEntity.AcceptChanges();
            //Add ManufactureEntity into DataEntity
            DataEntity.Tables.Add(ManufactureEntity);
        }
        #endregion
        ...
        //Implements from IManufactureView
        public void InitView()
        {
            ...
            Initialize();
            //Lấy về danh sách các Hãng SX để hiển thị lên DataGridView
            GetData();
        ...
        private bool IsValidData()
        {
            ...
            {
               mdlStatic.SetMsg(lblMsg,"Bạn cần nhập mã Hãng SX.",true);
                txtCode.Focus();
                return false;
            }
            ...
            if (String.IsNullOrEmpty(txtName.Text))
            {
               ...
               mdlStatic.SetMsg(lblMsg,"Bạn cần nhập tên Hãng SX.",true);
                txtName.Focus();
                return false;
            }
            return true;
        ...
        private void SetControlStatus()
        {
            ...
            {
                case action.Insert:
                    //Cho phép nhập liệu mã Hãng SX,vị trí, tên Hãng SX và mô tả thêm
                    txtID.Text = "Tự sinh";
                    txtCode.Clear();
                    txtName.Clear();
                    txtDesc.Clear();
                    ...
                    txtPos.Text = Utility.getNextMaxID("Pos", "L_ManufactureList").ToString();
                    ...
                    break;
                case action.Update:
                    //Không cho phép cập nhật lại mã Hãng SX
                    Utility.DisabledTextBox(txtID);
                    //Cho phép cập nhật lại vị trí, tên Hãng SX và mô tả thêm
                    Utility.EnabledTextBox(txtCode);
                    Utility.EnabledTextBox(txtName);
                    Utility.EnabledTextBox(txtDesc);
                    Utility.EnabledTextBox(txtPos);
                ...
                case action.FirstOrFinished://Hủy hoặc trạng thái ban đầu khi mới hiển thị Form
                    //Không cho phép nhập liệu mã Hãng SX, tên Hãng SX và mô tả thêm
                    Utility.DisabledTextBox(txtID);
        ...
        public void CurrentCellChanged()
        {
            try
            {
                ...
                if (bLoaded && AllowCurrentCellChangedOnGridView && grdList.RowCount > 0 && grdList.CurrentRow != null)
                {
                    ...
                    {
                        txtID.Text = Utility.GetValueFromGridColumn(grdList, "colManufacture_ID", grdList.CurrentRow.Index);
                        txtCode.Text = Utility.GetValueFromGridColumn(grdList, "colManufacture_Code", grdList.CurrentRow.Index);
                        txtName.Text = Utility.GetValueFromGridColumn(grdList, "colManufacture_Name", grdList.CurrentRow.Index);
        ...
        private void ExecuteAction()
        {
            ...
            //Gán ManufactureEntity vào DataEntity
            SetValueforEntity();

            //Khởi tạo BusinessRule để xử lý nghiệp vụ
            ManufactureInfor Infor = new ManufactureInfor();
            Utility.MapValueFromEntityIntoObjectInfor(Infor, ManufactureEntity);
            ManufactureController _BusRule = new ManufactureController(Infor);
            switch (Act)
            {
                case action.Insert:
                    ...
                    if (InsertResult == ActionResult.Success)//Nếu thành công
                    {
                        ...
                        //phải đảm bảo Datasource và ManufactureEntity có cấu trúc giống nhau mới dùng hàm này
                        DataRow dr = Utility.CopyData(ManufactureEntity.Rows[0], DataSource);
                        ...
                        Utility.GotoNewRow(grdList, "colManufacture_ID", Infor.Manufacture_ID.ToString());
                    ...
                    else//Có lỗi xảy ra
                    {
                        switch (InsertResult)
                        {
                            case ActionResult.ExistedRecord:
                                mdlStatic.SetMsg(lblMsg, "Đã tồn tại Hãng SX có mã: " + txtCode.Text.Trim() + ". Đề nghị bạn xem lại",true);
                               Utility.FocusAndSelectAll( txtCode);
                                break;
                            default:
                               mdlStatic.SetMsg(lblMsg,"Lỗi trong quá trình thêm mới Hãng SX. Liên hệ với VBIT",true);
                                break;
                        }
                    }
                    break;
                case action.Update:
                    ...
                    if (UpdateResult == ActionResult.Success)//Nếu thành công
                    {
                        ...
                        DataRow dr = Utility.GetDataRow(DataSource, "Manufacture_ID", txtID.Text.Trim());
                        if (dr != null)
                        {
                            Utility.CopyData(ManufactureEntity.Rows[0], ref dr);
                            DataSource.AcceptChanges();
                        }
                        //Return to the InitialStatus
                        Act = action.FirstOrFinished;
                        //Nhảy đến bản ghi vừa cập nhật trên lưới. Do txtID chưa bị reset nên dùng luôn
                        Utility.GotoNewRow(grdList, "colManufacture_ID", txtCode.Text.Trim());
                    ...
                    else//Có lỗi xảy ra
                    {
                        switch (UpdateResult)
                        {
                            case ActionResult.Error:
                               mdlStatic.SetMsg(lblMsg,"Lỗi khi cập nhật Hãng SX. Liên hệ với VBIT",true);
                                break;
                            default:
                               mdlStatic.SetMsg(lblMsg,"Lỗi khi cập nhật Hãng SX. Liên hệ với VBIT",true);
                                break;
                ...
                case action.Delete:
                    if (Utility.AcceptQuestion("Bạn có muốn xóa Hãng SX đang chọn hay không?", "Xác nhận xóa", true))
                    {
                        string Manufacture_ID = txtID.Text.Trim();
                        ...
                        if (DeleteResult == ActionResult.Success)//Nếu xóa thành công trong CSDL
                        {
                            ...
                            DataRow dr = Utility.GetDataRow(DataSource, "Manufacture_ID", Manufacture_ID);
                            if (dr != null)
                            {
                            ...
                            //Return to the InitialStatus
                            Act = action.FirstOrFinished;
                            mdlStatic.SetMsg(lblMsg, "Đã xóa Hãng SX có ID: " + Manufacture_ID + " ra khỏi hệ thống.", false);
                            SetControlStatus();
                            CurrentCellChanged();
                        }
                        else//Có lỗi xảy ra
                        {
                            switch (DeleteResult)
                            {
                                case ActionResult.DataHasUsedinAnotherTable:
                                    mdlStatic.SetMsg(lblMsg, "Hãng SX có ID: " + Manufacture_ID + " đã được sử dụng trong bảng khác nên bạn không thể xóa!", true);
                                    break;
                                default:
                                   mdlStatic.SetMsg(lblMsg,"Lỗi khi xóa Hãng SX. Liên hệ với VBIT",true);
                                    break;
                            }
                        }
                    }
        ...
        /// <summary>
        /// Lấy danh sách Hãng SX và Binding vào DataGridView
        /// </summary>
        private void GetData()
        {
            DataSource = new ManufactureController().GetAllData().Tables[0];
            Utility.SetDataSourceForDataGridView(grdList, DataSource, false, true,"", "Pos,Manufacture_Name");
        }
        #endregion
        ...
        private void SearchData(string _ID, string _Code, string _Name)
        {
            ...
            if (_ID.Trim() != "")
                WhereCondition = " Manufacture_ID=" + Utility.Int16Dbnull(_ID) ;
            if(_Code.Trim()!="")
                if (WhereCondition != "") WhereCondition += " AND Manufacture_Code='" + _Code + "'";
                else
                    WhereCondition = " Manufacture_Code='" + _Code + "'";
                
            if (_Name.Trim() != "")
                if (WhereCondition != "") WhereCondition += " AND Manufacture_Name like '%" + _Name + "%'";
                else
                    WhereCondition = " Manufacture_Name like '%" + _Name + "%'";
            if (_ID.Trim()=="" &&  _Code.Trim() == "" && _Name.Trim() == "") WhereCondition = " 1=1";
            DataSource = new ManufactureController().GetData(WhereCondition).Tables[0];
            Utility.SetDataSourceForDataGridView(grdList, DataSource, false, true, "", "Pos,Manufacture_Name");
            CurrentCellChanged();
        }
    }
