...
{
    public partial class frm_ChooseProcedureList : Form
    {
        #region "Attributes"
        ...
        #endregion
        public frm_ChooseProcedureList()
        {
            InitializeComponent();
            ProList = "";
        ...
        private void frm_ChooseProcedureList_Load(object sender, EventArgs e)
        {
            InitView();
        }
            ...
            foreach (TreeNode _Node in tvwProcedure.Nodes)
            {
                ...
                else//Cos con
                {
                    foreach (TreeNode _Node1 in _Node.Nodes)
                    {
                        if (_Node1.Checked)
                        {
                            ProList += _Node1.Text.Split('-')[0] + ";";
                            _Name = _Node1.Tag.ToString().Split('@')[1].Split('^')[0];
                             Standard_Name = _Name.Split('|')[0];
                             Display_Name = _Name.Split('|')[1];
                             DirectionCapture = _Node1.Tag.ToString().Split('@')[1].Split('^')[1];
                             arrProc.Add("#" + _Node1.Tag.ToString().Split('#')[0] + "#" + _Node1.Text.Split('-')[0] + "#" + Standard_Name + "#" + Display_Name + "#" + DirectionCapture);
                        }
                    }
                }
            }
